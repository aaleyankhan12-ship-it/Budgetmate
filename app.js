...
async function handleLogin(event) {
  ...
  if (error) {
    els.loginError.textContent = error.message || "Login failed. Please try again.";
    return;
  }
...
async function handleSignup(event) {
  ...
  if (error) {
    els.signupError.textContent = error.message || "Sign up failed. Please try again.";
    return;
  }

  if (!data.session) {
    els.signupError.textContent = "Account created. Please confirm your email, then log in.";
    showAuthForm("login");
    return;
  }
